...
Library
/
GST_Statewise_Analysis_and_Clustering_updated.md
...
* [Methodology](#methodology)
  ...
  * [2. Data Visualization](#2-data-visualization)
    ...
    * [GST Collection Across Years — Heatmap](#21-gst-collection-across-years--heatmap)
    * [State-wise GST Collection — 4-Year Comparison](#22-state-wise-gst-collection--4-year-comparison)
    * [Mean GST Collection per Year](#23-mean-gst-collection-per-year)
    * [Maharashtra vs Karnataka](#24-gst-collection-maharashtra-vs-karnataka)
    * [Year-wise Total GST Collection](#25-year-wise-total-gst-collection)
    * [GST State Clusters](#26-gst-state-clusters)
  * [3. Elbow Method vs. Silhouette Score](#3-elbow-method-vs-silhouette-score)
  * [4. GST State Clusters — K-Means](#4-gst-state-clusters--k-means)

    * [Normal Clustering](#41-normal-gst-state-clustering)
    * [Growth Clustering](#42-growth-based-gst-state-clustering)
    * [Volatility Clustering](#43-volatility-based-gst-state-clustering)
* [Technologies Used](#technologies-used)
* [Reproducing the Analysis](#reproducing-the-analysis)
* [Data Source](#data-source)
* [Author](#author)
...
## 1. GST collections increased over the study period

Mean GST collection increased from approximately **₹22,201 crore in FY 2020–21** to **₹39,058 crore in FY 2023–24**.

The visualization of mean GST collection shows a consistent upward trend across the four financial years.

## 2. GST collection varies substantially between jurisdictions

The state-wise visualizations show considerable differences in GST collection levels.

A small number of major states contribute substantially higher amounts than smaller states and Union Territories.

## 3. GST collection is strongly right-skewed

The descriptive statistical analysis indicates a positively skewed distribution, reflecting the presence of a small number of very high-revenue jurisdictions.

## 4. Revenue level and growth represent different patterns

Normal clustering groups jurisdictions primarily according to their absolute GST collection levels, whereas growth clustering identifies similarities in their YoY growth trajectories.

Therefore, a jurisdiction can have relatively low revenue but belong to a high-growth group.

## 5. Small revenue bases can produce extreme growth rates

Lakshadweep is identified as a growth outlier, demonstrating the sensitivity of percentage growth to small initial values.

## 6. Volatility differs substantially between jurisdictions

Most jurisdictions exhibit comparatively low volatility, while a small number appear as volatility outliers.

Administrative changes also need to be considered when interpreting extreme volatility.

## 7. PCA highlights the dominant dimensions of the data

For normal GST revenue clustering, the first principal component accounts for almost all of the variance, showing that overall fiscal scale dominates the variation in annual revenue.

For growth clustering, the first two principal components together explain approximately **85.9% of the variance**, indicating a more multidimensional growth structure.

## 8. Volatility clustering provides the strongest separation

The silhouette score is highest for the volatility-based clustering:

| Approach         | Silhouette Score |
| ---------------- | ---------------: |
| Normal / Revenue |            0.663 |
| Growth / YoY     |            0.718 |
| Volatility       |        **0.834** |

This indicates that the volatility-based groups are the most clearly separated among the three clustering approaches.
